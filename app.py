...
# Path to the default example .plsql file
default_file_path = "https://github.com/chun-yu-ko/ios_power_log_explorer/blob/main/powerlog_YYYY-MM-DD_Example.PLSQL"
...
# Button to switch to using the example file
if uploaded_file is not None:
    file_path = "/tmp/uploaded_file.plsql"
    with open(file_path, "wb") as f:
        f.write(uploaded_file.getbuffer())
else:
    file_path = default_file_path

# Determine which file to use based on user input or button press
if uploaded_file is not None:
    file_path = "/tmp/uploaded_file.plsql"
    with open(file_path, "wb") as f:
        f.write(uploaded_file.getbuffer())
...
default_node = "__GLOBAL__"
default_node = "com.facebook.Facebook" if uploaded_file is None and default_node in node_options else node_options[0]
selected_node = st.selectbox("Select a Node", node_options, index=list(node_options).index(default_node))
